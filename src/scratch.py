...
from pool import HttpPool, HttpsPool
...
class Scratcher(object):
    ...
    def scratch_msgboard(self, logfile):
        real_args = copy.deepcopy(self.args_template)
        real_args["host"] = "m.qzone.qq.com"
        real_args["headers"] = {"Host": real_args["host"], "Cookie": self.config.COOKIE}
        real_args["logfile"] = logfile
        real_args["num"] = 10
        real_args["scratch_item"] = "msgboard"
        real_args["url_template"] = "/cgi-bin/new/get_msgb?" \
            + "uin=%s&hostUin=%s" % (self.config.G_QQ, self.config.H_QQ) \
            + "&inCharset=%s&outCharset=%s&format=%s" % (real_args["code"], real_args["code"], "jsonp") \
            + "&g_tk=%s" % self.config.G_TK \
            + "&num=%d&start=%d"
        
        real_args["mark1"] = "total\":"
        real_args["mark2"] = ","   
        
        self.scratch_template(real_args)

    def scratch_bloglist(self, logfile):
        real_args = copy.deepcopy(self.args_template)
    ...
    def scratch_blogcmt(self, blogid, logfile, append = False):
        real_args = copy.deepcopy(self.args_template)
        real_args["host"] = "b1.qzone.qq.com"
        real_args["headers"] = {"Host": real_args["host"], "Cookie": self.config.COOKIE}
        real_args["logfile"] = logfile
        ...
        real_args["url_template"] = "/cgi-bin/blognew/get_comment_list?" \
            ...
            + "&inCharset=%s&outCharset=%s&format=%s" % (real_args["code"], real_args["code"], "jsonp") \
            + "&g_tk=%s" % self.config.G_TK \
            + "&topicId=%s_%s" % (self.config.H_QQ, blogid) \
            + "&num=%d&start=%d"
    ...
    def scratch_shuoshuo(self, logfile):
        real_args = copy.deepcopy(self.args_template)
    ...
    def scratch_albumlist(self, logfile):
        real_args = copy.deepcopy(self.args_template)
        ...
        for host in album_hosts:  
            ...
            ret, n = self.scratch_template(real_args)
            if ret and n > 0:
                break

    def scratch_photolist(self, albumid, logfile, append = False):
        real_args = copy.deepcopy(self.args_template)
    ...
    def scratch_photocmt(self, albumid, photoid, logfile, append = False):
        real_args = copy.deepcopy(self.args_template)
    ...
    def scratch_template(self, args):
        host = args["host"]
        if args["connection_type"] == "HTTPS":
            conn = HttpsPool.get_instance().get_connection(host)
        else:
            conn = HttpPool.get_instance().get_connection(host)
        ...
        while start < total:
            ...
            try:
                ...
                time.sleep(1)
                continue     
            f.write(content.encode(self.config.code)) # encoding the same as the editor, or failed to open and analyze
            f.write("\n")
...
if __name__ == '__main__':
    ...
    scratcher = Scratcher(config)
    scratcher.scratch_msgboard(config.rawfile["msgboard"])
